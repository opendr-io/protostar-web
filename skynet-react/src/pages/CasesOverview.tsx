...
export function CasesOverview({ users, contentSections, ToggleWindow, isUserListOpen, setIsUserListOpen, isEntitySelected, setIsEntitySelected, setSelected } : CasesOverviewProps)
{
  ...
  function FilterCasesBasedOnPriority(priority: string)
  {
  ...
  return (
    <main>
      ...
      <div onClick={() => ToggleWindow(isUserListOpen, setIsUserListOpen)} className="bg-black text-white mx-4 border border-gray-300 mt-4 py-2 p-4 rounded-md hover:bg-gray-600 font-normal cursor-pointer">
        <p className="cursor-pointer select-none" title="Click to open dropdown to select a priority to filter by">Priority</p>
          ...
          <div>
            <ul className="list-inside space-y-4 list-none">
              {uniquePriorities.map((priority) => (
                <li onClick={() => FilterCasesBasedOnPriority(priority)} className="text-gray-800 cursor-pointer px-4 py-2 hover:bg-gray-200 active:bg-gray-400">
                  <span>{priority}</span>
                </li>
              ))}
            </ul>
        ...
        {filterPriority ? (
          contentSections.filter(({ priority }) => priority === filterPriority).map(({ case_id, assigned_user, casename, description, priority, investigated_entity }) => (
            <div
              ...
              onClick={() => 
              {
                setSelected({ case_id, casename, description, assigned_user, priority, investigated_entity });
          ...
          contentSections.map(({ case_id, assigned_user, casename, description, priority, investigated_entity }) => (
            <div
              key={case_id}
              className="bg-[#1B1B1B] text-white rounded-lg shadow cursor-pointer aspect-square p-6 flex flex-col"
              onClick={() => 
