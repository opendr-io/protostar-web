...
export function Home()
{
  ...
  useEffect(() =>
  {
    ...
    async function FetchData()
    {
      ...
      if(storedSummary)
      {
        setTacticalSummary(storedSummary);
      }
      else
      {
        let firstOutput = await llm.AskClaude(ps.ThreatStatusSummaryPrompt(highLevelDataList));
        localStorage.setItem('threatstatussummary', firstOutput);
        let summaryOutput = await llm.AskClaude(ps.SummaryOfThreatStatusSummaryPrompt(firstOutput));
  ...
  return (
    <div className='bg-black text-white ml-10 min-h-screen mt-20'>
      <h1 className="text-3xl font-bold mt-4">Welcome to Skynet</h1>
      <div className='mt-4'>
        <h1>Briefing on Tactical Page</h1>
        <textarea readOnly={true} placeholder={tacticalSummary} className="bg-[#1B1B1B] w-[60rem] h-[30rem] text-gray-200 border-gray-300 overflow-y-auto cursor-default my-3 shadow resize-none appearance-none border rounded py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" />
      </div>
    </div>
  )
}